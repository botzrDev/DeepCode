...
class ContentGenerationAgent:
    ...
    async def generate_content(
        ...
        try:
            ...
            generated_content['cross_platform_tips'] = self._get_cross_platform_tips(platforms)
            generated_content['content_variations'] = self._suggest_content_variations(generated_content)
    ...
    async def _generate_platform_content(
        ...
        try:
            ...
            if hasattr(self.mcp_agent, 'call_tool'):
                prompt = self._build_generation_prompt(platform, intent_analysis, strategy)
            ...
            self.logger.warning(f"MCP content generation failed for {platform}: {str(e)}")
        
        # Fallback to built-in generation
        return self._generate_platform_content_fallback(platform, intent_analysis, strategy)
    ...
    def _parse_generation_response(self, response: str, platform: str) -> Dict[str, Any]:
        """
        ...
        try:
            if response.strip().startswith('{'):
                ...
                if 'platform_content' in parsed and platform in parsed['platform_content']:
    ...
    def _extract_content_from_text(self, response: str, platform: str) -> Dict[str, Any]:
        """
        ...
        content['hashtags'] = hashtags[:self.platform_specs.get(platform, {}).get('optimal_hashtags', 3)]
    ...
    def _generate_platform_content_fallback(
        ...
        templates = {
            'twitter': {
                'main_content': self._generate_twitter_content(intent_summary, topics, tone, cta),
            ...
            'instagram': {
                'main_content': self._generate_instagram_content(intent_summary, topics, tone, cta),
            ...
            'linkedin': {
                'main_content': self._generate_linkedin_content(intent_summary, topics, tone, cta),
            ...
            'facebook': {
                'main_content': self._generate_facebook_content(intent_summary, topics, tone, cta),
                'hashtags': self._generate_hashtags(topics, 2),
                'emojis': ['👍', '🎉', '📢'],
            ...
            'youtube': {
                ...
                'description': self._generate_youtube_description(intent_summary, topics, cta),
                'hashtags': self._generate_hashtags(topics, 8),
                'emojis': ['🎬', '⭐', '🔥'],
    ...
    def _generate_twitter_content(self, intent: str, topics: List[str], tone: str, cta: str) -> str:
    ...
    def _generate_instagram_content(self, intent: str, topics: List[str], tone: str, cta: str) -> str:
    ...
    def _generate_linkedin_content(self, intent: str, topics: List[str], tone: str, cta: str) -> str:
    ...
    def _generate_facebook_content(self, intent: str, topics: List[str], tone: str, cta: str) -> str:
    ...
    def _generate_youtube_description(self, intent: str, topics: List[str], cta: str) -> str:
        ...
        if cta:
            description += f"{cta}\n\n"
        description += "Don't forget to like, subscribe, and hit the notification bell! 🔔"
        return description
    ...
    def _generate_hashtags(self, topics: List[str], count: int) -> List[str]:
        ...
        for topic in topics:
            ...
            hashtag = '#' + topic.lower().replace(' ', '').replace('-', '').replace('_', '')
            if len(hashtag) > 2:  # Ensure hashtag is meaningful
                hashtags.append(hashtag)
        
        # Add some general hashtags if we need more
        general_hashtags = ['#content', '#socialmedia', '#marketing', '#engagement', '#digital']
    ...
    def _suggest_content_variations(self, generated_content: Dict[str, Any]) -> Dict[str, List[str]]:
        """
