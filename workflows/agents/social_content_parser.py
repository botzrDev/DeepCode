...
class SocialContentParser:
    ...
    async def parse_social_content(
        ...
        try:
            ...
            if metrics:
                analysis['engagement_analysis'] = self._analyze_engagement_metrics(metrics)
            
            # Perform trend analysis if comprehensive
            if analysis_type in ['comprehensive', 'competitive']:
                analysis['trend_analysis'] = await self._analyze_content_trends(content_data)
    ...
    def _analyze_tone(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ['excited', 'amazing', 'awesome', 'love', '!', '🎉', '🚀']):
            return 'enthusiastic'
        elif any(word in text_lower for word in ['professional', 'industry', 'business', 'strategy']):
            return 'professional'
        elif any(word in text_lower for word in ['question', '?', 'what do you think', 'thoughts']):
            return 'inquisitive'
    ...
    def _analyze_sentiment(self, text: str) -> str:
        ...
        positive_words = ['love', 'great', 'awesome', 'amazing', 'excellent', 'wonderful', 'fantastic', 'perfect', 'best', 'good', 'happy', 'excited']
        negative_words = ['hate', 'terrible', 'awful', 'horrible', 'worst', 'bad', 'sad', 'angry', 'disappointed', 'frustrated']
    ...
    def _analyze_hashtags(self, hashtags: List[str]) -> Dict[str, Any]:
        ...
        if len(hashtags) == 0:
            analysis['recommendations'].append('Consider adding relevant hashtags to increase discoverability')
        elif len(hashtags) > 10:
            analysis['recommendations'].append('Consider reducing hashtag count for better engagement')
        else:
            analysis['recommendations'].append('Good hashtag usage - consider testing variations')
    ...
    def _assess_trending_potential(self, hashtags: List[str]) -> str:
        ...
        trending_indicators = ['trend', 'viral', 'breaking', 'new', 'latest', '2024', '2025']
    ...
    def _analyze_media_content(self, media_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for media_item in media_data:
            ...
            analysis['format_distribution'][media_format] = analysis['format_distribution'].get(media_format, 0) + 1
            analysis['total_size'] += media_size
        
        # Generate media recommendations
        if analysis['media_count'] == 0:
            analysis['recommendations'].append('Consider adding visual content to increase engagement')
        elif analysis['media_count'] > 5:
            analysis['recommendations'].append('Multiple media items - ensure they add value to the message')
        else:
    ...
    def _analyze_engagement_metrics(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'engagement_quality': self._assess_engagement_quality(likes, comments, shares),
    ...
    def _generate_content_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if engagement_analysis.get('performance_level') == 'poor':
            ...
            recommendations.append('Excellent performance - consider replicating this content style')
    ...
    def _get_default_parsing_result(self, content_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'hashtag_analysis': {},
            'media_analysis': {},
            'recommendations': ['Content parsing encountered errors - manual review recommended']
        }