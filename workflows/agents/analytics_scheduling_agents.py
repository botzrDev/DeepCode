...
class AnalyticsAgent:
    ...
    async def analyze_performance(
        ...
        try:
            ...
            if not content_history:
                analysis['recommendations'].append("No historical data available for analysis")
                return analysis
            ...
            for platform in platforms:
                platform_data = [c for c in content_history if c.get('platform') == platform]
                analysis['platform_performance'][platform] = self._analyze_platform_performance(platform_data)
            
            # Find top performing content
            analysis['top_performing_content'] = self._identify_top_content(content_history)
            
            # Analyze content types
            analysis['content_type_analysis'] = self._analyze_content_types(content_history)
            
            # Track engagement trends
            analysis['engagement_trends'] = self._analyze_engagement_trends(content_history)
    ...
    def _analyze_platform_performance(self, platform_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        total_comments = sum(c.get('metrics', {}).get('comments', 0) for c in platform_data)
        ...
        avg_engagement = (total_likes + total_comments + total_shares) / len(platform_data) if platform_data else 0
        ...
        return {
            ...
            'engagement_rate': self._calculate_engagement_rate(total_likes + total_comments + total_shares, total_views),
            'best_posting_time': self._find_best_posting_time(platform_data),
            'top_hashtags': self._find_top_hashtags(platform_data)
        }
    
    def _calculate_engagement_rate(self, total_engagement: int, total_views: int) -> float:
        ...
        for content in platform_data:
            ...
            if posted_at:
                try:
                    ...
                    hour = dt.hour
                    engagement = content.get('metrics', {}).get('likes', 0) + content.get('metrics', {}).get('comments', 0)
    ...
    def _find_top_hashtags(self, platform_data: List[Dict[str, Any]]) -> List[str]:
        ...
        sorted_hashtags = sorted(hashtag_performance.keys(),
                               key=lambda h: sum(hashtag_performance[h]) / len(hashtag_performance[h]),
                               reverse=True)
        
        return sorted_hashtags[:5]  # Top 5 hashtags
    
    def _identify_top_content(self, content_history: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for content in content_history:
            ...
            engagement_score = metrics.get('likes', 0) + metrics.get('comments', 0) + metrics.get('shares', 0)
    ...
    def _analyze_content_types(self, content_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def _analyze_engagement_trends(self, content_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        first_half_avg = sum(c['engagement'] for c in dated_content[:mid_point]) / mid_point
        second_half_avg = sum(c['engagement'] for c in dated_content[mid_point:]) / (len(dated_content) - mid_point)
    ...
    def _generate_analytics_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        for platform, perf in analysis.get('platform_performance', {}).items():
            ...
            if engagement_rate < 2:
                recommendations.append(f"Improve {platform} engagement rate (currently {engagement_rate}%)")
            elif engagement_rate > 5:
                recommendations.append(f"Excellent {platform} performance! Consider increasing posting frequency")
        
        # Content type recommendations
        content_analysis = analysis.get('content_type_analysis', {})
        if content_analysis:
            best_type = max(content_analysis.keys(), 
                          key=lambda t: content_analysis[t].get('avg_engagement', 0))
            recommendations.append(f"Focus more on '{best_type}' content (highest engagement)")
        
        # Trend recommendations
        trends = analysis.get('engagement_trends', {})
        if trends.get('trend') == 'declining':
            recommendations.append("Engagement is declining - consider refreshing content strategy")
        elif trends.get('trend') == 'improving':
            recommendations.append("Engagement is improving - maintain current content approach")
        
        # General recommendations
        if analysis.get('total_posts', 0) < 10:
            recommendations.append("Increase posting consistency for better analytics insights")
        
        return recommendations
    
    async def generate_report(self, analysis: Dict[str, Any], format_type: str = "summary") -> str:
        """
...
"""
    ...
    def _get_default_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'audience_insights': {},
            'recommendations': ['No data available for analysis - start posting content to track performance']
        }
...
class SchedulingAgent:
    ...
    async def schedule_content(
        ...
        try:
            ...
            scheduling_result['recommendations'] = self._generate_scheduling_recommendations(
                scheduling_result, platforms
            )
    ...
    def _get_scheduling_reasoning(self, platform: str, scheduled_time: datetime) -> str:
        ...
        return reasoning_templates.get(platform, f"Scheduled for {hour}:00 based on general best practices")
    ...
    def _generate_scheduling_recommendations(
        ...
        if len(platforms) > 3:
            recommendations.append("Consider staggering posts across platforms to manage workflow")
        
        if scheduling_result.get('rate_limit_warnings'):
            recommendations.append("Monitor rate limits and spread content across time periods")
