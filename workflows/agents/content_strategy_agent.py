...
class ContentStrategyAgent:
    ...
    async def plan_content_strategy(
        ...
        try:
            ...
            strategy = {
                ...
                'posting_frequency': self._recommend_posting_frequency(platforms, urgency),
            ...
            for platform in platforms:
                strategy['platform_strategies'][platform] = await self._develop_platform_strategy(
                    platform, intent_analysis, platform_preferences
                )
            
            # Generate strategic recommendations
            strategy['recommendations'] = self._generate_recommendations(strategy, intent_analysis)
            
            self.logger.info(f"Content strategy developed for {len(platforms)} platforms")
            return strategy
    ...
    async def _develop_platform_strategy(
        ...
        platform_strategies = {
            ...
            'linkedin': {
                ...
                'content_types': ['professional updates', 'industry insights', 'thought leadership']
            },
            'facebook': {
                ...
                'content_types': ['updates', 'events', 'community posts', 'live videos']
            },
            'youtube': {
                ...
                'engagement_features': ['playlists', 'community tab', 'premieres', 'shorts'],
                'content_types': ['tutorials', 'vlogs', 'shorts', 'live streams']
            }
        }
        
        base_strategy = platform_strategies.get(platform, {
        ...
        base_strategy['target_audience'] = intent_analysis.get('audience', 'General audience')
    ...
    def _recommend_posting_frequency(self, platforms: List[str], urgency: str) -> Dict[str, str]:
        """
        ...
        if urgency == 'immediate':
            ...
            return {platform: multiplier_map.get(platform, base_frequencies.get(platform, '1 time per day')) 
                   for platform in platforms}
        
        return {platform: base_frequencies.get(platform, '1 time per day') for platform in platforms}
    
    def _develop_content_mix(self, content_type: str, platforms: List[str]) -> Dict[str, int]:
        """
    ...
    def _suggest_engagement_tactics(self, platforms: List[str]) -> List[str]:
        ...
        platform_specific_tactics = {
            'twitter': ['Use trending hashtags', 'Participate in Twitter chats', 'Retweet with comments'],
            'instagram': ['Use Instagram Stories polls', 'Share behind-the-scenes content', 'Collaborate with influencers'],
            'linkedin': ['Share industry insights', 'Publish thought leadership articles', 'Engage in professional groups'],
            'facebook': ['Create Facebook events', 'Share in relevant groups', 'Use Facebook Live'],
            'youtube': ['Create playlists', 'Use end screens and cards', 'Engage with comments']
        }
    ...
    def _define_success_metrics(self, intent_analysis: Dict[str, Any]) -> Dict[str, str]:
        """
    ...
    def _check_brand_alignment(self, brand_guidelines: Dict[str, Any] = None) -> Dict[str, Any]:
        """
    ...
    def _generate_recommendations(self, strategy: Dict[str, Any], intent_analysis: Dict[str, Any]) -> List[str]:
        """
        Generate strategic recommendations.
        ...
        if len(platforms) > 3:
            recommendations.append('Consider focusing on fewer platforms initially for better quality')
        
        if 'immediate' in intent_analysis.get('urgency', ''):
            recommendations.append('Plan for rapid content creation and approval process')
    ...
    def _get_default_strategy(self, intent_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'brand_alignment': {'status': 'default', 'message': 'Using default strategy'},
            'platform_strategies': {},
